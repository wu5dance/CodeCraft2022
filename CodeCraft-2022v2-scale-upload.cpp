...
int Read_Client(vector<CN>& c, string filename, vector<EN>& e, vector<vector<Qos>>& q, string fileSolution, int qos_max)//读取demand.csv，并为每时刻进行分配
{
    ...
    while (getline(file_t, lineStr) && file.good())//good()：文件没有发生任何错误时返回true
    {
        t_sum++;
    }
    file_t.close();
...
int main()
{
    ...
    init_Client(c, fileDemand);//初始化客户节点
    //cout << "客户节点数目：" << M << endl;
    //for (int i = 0; i < M; i++)
    //{
    //    cout << i << "," << c[i].id << " ";
    ...
    init_Edege(e, fileSiteBD);//初始化边缘节点
    //cout << "边缘节点数目：" << N << endl;
    ...
    //}
     ...
     init_Q(q, c, e);//将qos初始化为一个M行N列的二维数组，但并不读入文件

    Read_Qos(q, fileQos, c, e);//读取qos文件
    //if (error == 1)
    //    cout << "qos表格读取成功！" << endl;
    ...
    //    cout << endl;
    //}
    for (long unsigned int i = 0; i < q.size(); i++)//对每个客户节点对应的与所有边缘节点的qos进行升序排序
    {
